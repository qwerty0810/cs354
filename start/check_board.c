...
int valid_board(int **board, int size) {
	//row validation
	//goes through rows of the given table
	...
	for(int i=0; i<size; i++){
		for (int j=0; j<size; j++){
			for (int k=j+1; k<size; k++){
				...
				//checks for repeated digits in the rows of the given table
				if(*(*(board+i)+j)==*(*(board+i)+k) && *(*(board+j)+k)!=0){
					//invalid if repeated numbers are found
					return 0;
				}
			}
		}
	...
	//column validation
	//goes through columns of the given table
	//if element =0, continue
	for(int i=0; i<size; i++){
		for(int j=0; j<size; j++){
			for(int k=j+1; k< size; k++){
				...
				//checks for repeated numbers in the columns of the given table
				if(*(*(board+j)+i)==*(*(board+k)+i) && *(*(board+i)+k)!=0){
					return 0;
					//invalid if found
				}
			}
	...
	return 1;
	//table is valid if no repeated numbers are found
}    
...
int main( int argc, char **argv ) {              
	...
	if(argc!=2){
		printf("Usage: ./checkboard <input_filename> \n");
		exit(1);
	}
